...
class PlayerControls:
   ...
   function_storage = []
   def player_operations(n, event, sympy_operation, enemy, equation_object, surface_image_equation):
      PlayerControls.n = n
      ...
      if func_graphable:
         if event.key == pygame.K_1:
            ...
            sympy_operation, surface_image_equation = OperationsSelf.operation_addition(equation_object, sympy_operation, n)
         elif event.key == pygame.K_2:
            SoundEffects.sound_effect_interaction.play()
            sympy_operation, surface_image_equation = OperationsSelf.operation_subtraction(equation_object, sympy_operation, n)
         elif event.key == pygame.K_3:
            SoundEffects.sound_effect_interaction.play()
            sympy_operation, surface_image_equation = OperationsSelf.operation_multiplication(equation_object, sympy_operation, n)
         elif event.key == pygame.K_4:
            SoundEffects.sound_effect_interaction.play()
            sympy_operation, surface_image_equation = OperationsSelf.operation_division(equation_object, sympy_operation, n)
         elif event.key == pygame.K_5:
            SoundEffects.sound_effect_interaction.play()
            sympy_operation, surface_image_equation = OperationsSelf.operation_to_power(equation_object, sympy_operation, n)
            ...
            SoundEffects.sound_effect_interaction.play()
            sympy_operation, surface_image_equation = OperationsSelf.operation_exponentiated(equation_object, sympy_operation, n)
         elif event.key == pygame.K_9:
            SoundEffects.sound_effect_interaction.play()
            sympy_operation, surface_image_equation = OperationsSelf.operation_root(equation_object, sympy_operation, n)
         elif event.key == pygame.K_0:
            SoundEffects.sound_effect_interaction.play()
            sympy_operation, surface_image_equation = OperationsSelf.operation_evaluate(equation_object, sympy_operation, n)
         return(sympy_operation, surface_image_equation)
      else:
         SoundEffects.sound_effect_error.play()
         return(sympy_operation, surface_image_equation)
   
   def player_func_draw(background_origin, sympy_operation, equation_object, enemy, color_default):
      try:
         ...
         check_hit = DrawFunction.render_graph(PlayerControls.function_storage, enemy_rect_hitbox, background_origin) #sends the updated enemy_rect to the render_graph function to check if the rect of the enemy collides with the function
         if check_hit == True:
